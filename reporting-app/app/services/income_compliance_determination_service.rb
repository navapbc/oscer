...
# Mirrors HoursComplianceDeterminationService: no mailers here; publishes income-specific Strata events from #determine for workflow/notifications when the income path runs.
...
class IncomeComplianceDeterminationService
  ...
  class << self
    # Called when the income CE path runs (e.g. future business-process wiring).
    # Publishes income-specific Strata events (parallel to hours’ DeterminedHours* / DeterminedActionRequired).
