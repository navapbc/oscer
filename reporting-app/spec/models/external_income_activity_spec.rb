...
# ExternalIncomeActivity is a read-only stub over a retained table; ExternalActivity supersedes it.
RSpec.describe ExternalIncomeActivity, type: :model do
  ...
  def insert_row(member_id: 'M12345')
    described_class.insert_all!([
      { member_id: member_id, category: 'employment', gross_income: 580,
        period_start: Date.new(2026, 1, 1), period_end: Date.new(2026, 1, 31),
        source_type: 'api', reported_at: Time.current,
  ...
  it 'refuses to create rows' do
    expect {
      described_class.create!(member_id: 'M12345', category: 'employment', gross_income: 580,
                              period_start: Date.new(2026, 1, 1), period_end: Date.new(2026, 1, 31),
                              source_type: 'api', reported_at: Time.current)
  ...
  it 'refuses to update existing rows' do
    ...
    expect { row.update!(gross_income: 10) }.to raise_error(ActiveRecord::ReadOnlyRecord)
  end
  ...
  it 'still reads retained rows by member' do
    ...
    expect(described_class.for_member('M12345').map(&:gross_income)).to eq([ 580 ])
  end
end
