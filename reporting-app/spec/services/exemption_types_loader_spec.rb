...
RSpec.describe ExemptionTypesLoader, type: :service do
  ...
  describe ".safe_load_optional" do
    ...
    context "when the YAML contains a disallowed Ruby tag" do
      ...
      it "raises ConfigurationError with 'Invalid YAML'" do
        ...
        }.to raise_error(ExemptionTypesLoader::ConfigurationError, /Invalid YAML/)
      end
    end
    ...
    context "when the YAML has a non-Hash top level" do
      ...
      it "raises ConfigurationError matching 'Expected a Hash at top level'" do
        ...
        }.to raise_error(ExemptionTypesLoader::ConfigurationError, /Expected a Hash at top level/)
      end
    end
  end
  ...
  describe ".merge_with_defaults" do
    context "with an empty override hash" do
      it "returns DEFAULTS verbatim" do
        expect(described_class.merge_with_defaults({})).to eq(ExemptionTypesLoader::DEFAULTS)
      end
    end

    context "with an override that disables a default" do
      it "deep-merges; override values win on shared keys" do
        result = described_class.merge_with_defaults("medical_condition" => { "enabled" => false })
        expect(result["medical_condition"]["enabled"]).to be false
        expect(result["caregiver_child"]["enabled"]).to be true
      end

      it "preserves all other defaults intact" do
        result = described_class.merge_with_defaults("medical_condition" => { "enabled" => false })
        expect(result.size).to eq(ExemptionTypesLoader::DEFAULTS.size)
    ...
    context "with an override that adds a new entry" do
      it "produces all defaults plus the new entry" do
        ...
        expect(result.size).to eq(ExemptionTypesLoader::DEFAULTS.size + 1)
  ...
  describe ".transform" do
    ...
    context "when an entry value is not a Hash" do
      it "raises ConfigurationError mentioning the offending id" do
        merged = { "medical_condition" => true }
        expect {
          described_class.transform(merged)
        }.to raise_error(ExemptionTypesLoader::ConfigurationError, /medical_condition/)
      end
    end

    context "when an entry is missing the enabled field" do
      it "raises ConfigurationError mentioning the offending id" do
        merged = { "medical_condition" => {} }
        expect {
          described_class.transform(merged)
        }.to raise_error(ExemptionTypesLoader::ConfigurationError, /medical_condition/)
      end
  ...
  describe "integration" do
    it "module-direct against real override path returns the 7 OSCER defaults, all enabled" do
