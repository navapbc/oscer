...
namespace :dev do
  namespace :member_dashboard do
    desc "List OSCER-337 member dashboard Figma frames for local QA"
    task frames: :environment do
      ...
      puts "Member dashboard frames (use with dev:member_dashboard:apply[EMAIL,FRAME]):"
      Dev::MemberDashboardFrameSetup.list_frames
      puts
      puts "Example:"
      puts "  docker compose exec reporting-app bin/rake 'dev:member_dashboard:apply[jake@test.com,get_started]'"
    end

    desc "Apply a member dashboard Figma frame for local QA (EMAIL,FRAME)"
