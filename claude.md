...
- **Activity Reporting** – `ActivityReportApplicationForm` → `Activity` (STI: `HourlyActivity`, `IncomeActivity`, `WorkActivity`) → `ActivityReportInformationRequest`
...
- **External Activities** – `ExternalActivity` carrying hours, gross income, or both on one row, with source tracking (batch upload vs. API); imported via batch or VA integration. `ExternalHourlyActivity` / `ExternalIncomeActivity` are read-only stubs over superseded tables
