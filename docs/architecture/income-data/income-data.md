...
4. At `EX_PARTE_COMMUNITY_ENGAGEMENT_CHECK_STEP`, `CertificationBusinessProcess` invokes `CommunityEngagementCheckService.determine(kase)`. That service aggregates hours (`HoursComplianceDeterminationService.aggregate_hours_for_certification`) and income (`IncomeComplianceDeterminationService.aggregate_income_for_certification`), then `CertificationCase#record_ex_parte_ce_combined_assessment` persists **one** automated determination with `calculation_type` `ex_parte_ce_combined` (nested hours/income payloads and `satisfied_by`). It then publishes generic community-engagement Strata events—`DeterminedCommunityEngagementMet` (either track passes), `DeterminedCommunityEngagementInsufficient` (both fail but some ex parte hours; payload includes `hours_data` and `income_data`), or `DeterminedCommunityEngagementActionRequired` (both fail and no ex parte hours)—which `CertificationBusinessProcess` transitions map to `end` or `report_activities`. Member is compliant if **either** track meets its threshold. Other flows (e.g. hours-only paths elsewhere) may still use `DeterminedHoursMet` / `DeterminedHoursInsufficient`; see `NotificationsEventListener`.
