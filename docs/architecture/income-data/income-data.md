...
```json
{
  ...
  "total_income": 620.0,
  "target_income": 580.0,
  "income_by_source": { "external": 620.0, "activity": 0.0 },
  "period_start": "2025-01-01",
  "period_end": "2025-01-31",
...
Hours-only and combined external CE use **`Determinations::HoursBasedDeterminationData`** and **`Determinations::ExternalCECombinedDeterminationData`** respectively; see `Determination` model comments for legacy vs current `calculation_type` values.
...
4. At `EXTERNAL_COMMUNITY_ENGAGEMENT_CHECK_STEP`, `CertificationBusinessProcess` invokes `CommunityEngagementCheckService.determine(kase)`.
   ...
   - `CertificationCase#record_external_ce_combined_assessment` persists **one** automated determination with `calculation_type` matching `Determination::CALCULATION_TYPE_EXTERNAL_CE_COMBINED` (`external_ce_combined`; nested hours/income payloads and `satisfied_by`). Member is compliant if **either** track meets its threshold. Historical rows may still use `Determination::CALCULATION_TYPE_EXTERNAL_CE_COMBINED_LEGACY` (`ex_parte_ce_combined`).
